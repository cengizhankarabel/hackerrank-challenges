...
public class Solution {
    ...
    private static boolean isWon(int _leap, int[] _game, int curr) {

        if (curr < 0 || _game[curr] == 1) {

            return false;
        }


        if ((curr == _game.length - 1) || curr + _leap > _game.length - 1) {
        ...
        return isWon(_leap, _game, curr + 1) || isWon(_leap, _game, curr - 1) || isWon(_leap, _game, curr + _leap);
    }
